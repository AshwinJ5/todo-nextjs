...
export default function EditTaskModal({ isOpen, onClose, task, onSave, onDelete }: EditTaskModalProps) {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            await todoService.updateTodo(String(task.id), payload);
            ...
            <div className="relative flex w-full max-w-lg flex-col rounded-xl bg-white dark:bg-zinc-900 shadow-2xl animate-in fade-in zoom-in-95 duration-200 m-4">
                <div className="flex items-center justify-between p-6 border-b border-slate-200 dark:border-zinc-800">
                    ...
                    <button
                        onClick={onClose}
                        disabled={isLoading}
                        className="p-1 rounded-full text-slate-500 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-zinc-800 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[#137fec] dark:focus:ring-offset-zinc-900 transition-colors disabled:opacity-50"
                    >
                        <svg
                    ...
                    <div className="flex flex-col">
                        ...
                        <div className="relative flex w-full items-stretch">
                            <input
                                ...
                                disabled={isLoading}
                                className="h-12 flex-1 rounded-l-lg border border-r-0 border-slate-300 bg-white p-4 text-base font-normal leading-normal text-slate-900 placeholder:text-slate-400 focus:border-[#137fec] focus:ring-1 focus:ring-[#137fec] outline-none dark:border-zinc-700 dark:bg-zinc-900 dark:text-slate-50 dark:placeholder:text-zinc-500 disabled:opacity-50"
                            />
                ...
                <div className="flex items-center justify-between p-6 border-t border-slate-200 dark:border-zinc-800">
                    <button
                        ...
                        disabled={isLoading}
                        className="px-4 py-2 text-sm font-semibold text-red-600 rounded-lg hover:bg-red-50 dark:text-red-500 dark:hover:bg-red-500/10 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 dark:focus:ring-offset-zinc-900 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                        Delete Task
                    </button>
                    <button
                        ...
                        className="flex items-center gap-2 px-6 py-2.5 text-sm font-semibold text-white bg-[#137fec] rounded-lg shadow-sm hover:bg-[#137fec]/90 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[#137fec] dark:focus:ring-offset-zinc-900 disabled:opacity-50 disabled:cursor-not-allowed transition-all min-w-[120px] justify-center"
                    >
                        {isLoading ? (
                            <svg
